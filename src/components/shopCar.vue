<template>
  <div class="shopcar">
    <div class="shop_content" v-if="this.$store.state.buyGoods.length !=0 ">
      <div class="goods_info" v-for="i in this.$store.state.buyGoods" :key="i.id">
      ...
      <div class="settle">
        <div class="settle_box">
          <div class="settle_info">
            ...
            <p>
              已勾选商品
              <span class="goods_amount">{{ this.$store.getters.getAllShopCarInfo.count }}</span>件，总价:
              <span class="goods_price">￥{{ this.$store.getters.getAllShopCarInfo.price }}</span>
            </p>
          </div>
